...
####################################卡券月报####################################
echo "开始从hdfs获取卡券月报"$former"数据"
#########卡券月报hdfs路径
ticket_report_month_hdfs=${rootPath_hdfs}/report_month/ttyh/${former}/ticket/*
hadoop fs -test -e ${ticket_report_month_hdfs}
if [ $? -eq 0 ]
        then
        echo ""
else
        echo "月报数据不存在"${ticket_report_month_hdfs}
        exit
fi
#########卡券月报本地路径
ticket_report_month_local=${rootPath_local}/ticket/report_month/${former}
#判断是否已存在本地目录
if [ ! -d "$ticket_report_month_local" ]; then  
    mkdir "$ticket_report_month_local"
else
    echo "月报本地目录已存在"$ticket_report_month_local
    exit 
fi
####################################商家卡片月报####################################
echo "开始从hdfs获取商家卡片月报"$former"数据"
#########商家卡片月报hdfs路径
shopcard_report_month_hdfs=${rootPath_hdfs}/report_month/ttyh/${former}/shopcard/*
hadoop fs -test -e ${shopcard_report_month_hdfs}
if [ $? -eq 0 ]
        then
        echo ""
else
        echo "月报数据不存在"${shopcard_report_month_hdfs}
        exit
fi
#########商家卡片月报本地路径
shopcard_report_month_local=${rootPath_local}/shopcard/report_month/${former}
#判断是否已存在本地目录
if [ ! -d "$shopcard_report_month_local" ]; then  
    mkdir "$shopcard_report_month_local"
else
    echo "月报本地目录已存在"$shopcard_report_month_local
    exit 
fi
...
hadoop fs -get ${ticket_report_month_hdfs} ${ticket_report_month_local}
hadoop fs -get ${shopcard_report_month_hdfs} ${shopcard_report_month_local}
...
echo "成功获取卡券月报数据"$ticket_report_month_local
echo "成功获取卡券月报数据"$shopcard_report_month_local
